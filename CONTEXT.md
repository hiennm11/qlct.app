...
|---|------|-------------------|------|
| 1 | **Merge categories** | ADR-0034 §Deferred | ADR-0034 close "hard delete" + "placeholder cleanup" nhưng defer "merge" (reassign all transactions từ cat A → cat B). |
...
| ADR-0037 | Auto-purge trash sau N ngày; merge 2 categories; placeholder category cleanup workflow; re-order archived section; bulk-archive categories |
...
- **Nhóm A (concrete)**: Merge categories (#1) — scope rõ, low risk (reuses existing `transaction.categoryId` mutation patterns). Recommend viết ADR mới.
