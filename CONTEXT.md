...
| **Tự động dọn thùng rác** | Auto-purge | ADR-0045 + ADR-0047: persisted setting `auto_purge_enabled` (SharedPreferences, default true). Khi ON, các danh mục trong thùng rác sẽ bị xoá vĩnh viễn sau 30 ngày. User toggle ở SettingsScreen. CategoryManagementScreen đọc setting để gate warning banner. |
...
43. **Settings: Auto-purge (UI move)** — ADR-0047: full-screen `SettingsScreen` (AppBar "Cài đặt" + ListView + section header "Dữ liệu") với auto-purge switch (move từ `CategoryManagementScreen` Trash section). Bug kèm: FAB `+` ở CategoryManagement che switch → giờ switch ở Settings screen riêng. Entry point: `HomeScreen` AppBar PopupMenuButton thêm "Cài đặt" giữa "Quản lý danh mục" và divider "Giới thiệu". Trash section giữ warning banner (gated on `_autoPurgeEnabled`) để user vẫn thấy items sắp bị purge. Known limitation: CategoryManagement load setting 1 lần ở `initState` — nếu user toggle setting ở Settings rồi return về CategoryManagement mà screen vẫn mounted, warning banner không reflect cho đến rebuild. Acceptable cho P3 #4; reopen trigger nếu user phàn nàn. Scaffold 1-section hiện tại — 6 P+ candidates (theme, currency, interval, voice lang, budget carry toggle, clear all) để grill + ADR riêng. No schema/dependency/release bump.
...
- **Known limitation (audit 2026-06-14)**: `CategoryManagementScreen._loadAutoPurgePref()` chỉ chạy 1 lần ở `initState`. Nếu user toggle setting ở Settings → return về CategoryManagement mà screen vẫn mounted, warning banner không reflect cho đến rebuild (vm change khác). Acceptable cho P3 #4 — reopen trigger nếu user phàn nàn. P+ fix: `RouteAware` hoặc expose `autoPurgeEnabled` qua Provider để rebuild reactive.
...
- **Post-ADR-0048 widget test drift (regression)** — detected 2026-06-14 sau commit `801ffe6`: full widget scope `+979 -14` — `budget_overview_widget_test.dart: (tearDownAll)` hang quá 10 phút → fanout timeout cho 14 test downstream. 1 riêng biệt: `category_management_screen_test.dart:108` `tapping a row opens edit bottom sheet` — `find.text('Thứ tự hiển thị')` không match (copy đã đổi sang `'Thứ tự'` shorthand). Pre-existing, không liên quan ADR-0048 (test mới của tôi `shows trash empty state` PASS). Housekeeping batch riêng theo `pre-existing-test-drift.md`. Reopen trigger: khi reopen sẽ fix (1) shorten test label `Thứ tự` thay vì `Thứ tự hiển thị`, (2) investigate `budget_overview_widget_test` `pumpAndSettle` hang root cause (timer? animation?). Ước lượng 1-2 day, có thể warrant riêng ADR nếu root cause sâu.
